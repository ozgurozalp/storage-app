<template>
	...
	>
		<template v-if="files.length === 0">
			...
			>
				<label :key="type.id" v-for="type in types" class="h-full w-full cursor-pointer">
					<input
						:value="type.id"
			...
			>
				...
				<template v-if="selectedTypeId === 1">
					...
					<p class="font-medium" v-else>Drag and drop some files here, or click to select files</p>
				</template>
				<p v-else class="font-medium">
					Paste your files here with
					<span class="inline-flex bg-gray-300 p-1 rounded">CTRL + V / ⌘ + V</span>
				</p>
			</div>
		</template>
		<section class="flex flex-col gap-3" v-else>
			...
			<div class="grid gap-2 overflow-hidden grid-cols-2 md:grid-cols-4">
				...
				>
					...
					>
						...
						<div class="flex flex-col gap-2 items-center justify-center" v-else>
							<div
								class="border-2 border-green-500 h-[50px] w-[50px] flex items-center justify-center rounded-full"
							>
								<Icon :size="30" class="text-green-500" name="checkmark-fill" />
							</div>
							<Button @click="copy(file.url)" class="text-sm whitespace-nowrap !px-2 !py-1"
								>Copy URL</Button
							>
						</div>
					</div>
			...
			<div class="grid grid-cols-2 gap-2" v-if="!files.every(file => !!file.progress)">
				<Button @click="cancel">Cancel</Button>
				<Button @click="upload" variant="primary">Upload</Button>
			</div>
			<Button variant="primary" @click="cancel" v-if="files.every(file => !!file.url)"
...
import altogic, { uploadFile } from '@/libs/altogic';
