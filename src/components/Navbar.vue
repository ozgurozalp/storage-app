	...
	<nav class="h-[70px] min-h-[70px]: bg-primary flex sticky top-0 z-[50]">
		<div class="flex items-center justify-between w-full px-2 md:px-4 lg:px-8">
			<div class="flex items-center gap-4">
				<router-link to="/" class="text-white font-bold whitespace-nowrap md:text-lg xl:text-2xl"
					>Image Uploader</router-link
				>
			</div>
			<div class="flex items-center lg:gap-1" v-if="!isAuthenticated">
				<router-link active-class="active" to="/login" class="menu-item">Sign in</router-link>
				<router-link active-class="active" to="/register" class="menu-item">Sign up</router-link>
			</div>
			<div class="flex items-center lg:gap-1" v-else>
				<router-link active-class="active" to="/dashboard" class="menu-item">Dashboard</router-link>
